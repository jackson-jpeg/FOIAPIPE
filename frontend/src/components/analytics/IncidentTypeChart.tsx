...
export function IncidentTypeChart({ data }: IncidentTypeChartProps) {
  const formatted = data.map(d => ({
    ...d,
    name: INCIDENT_TYPES[d.incident_type as keyof typeof INCIDENT_TYPES]?.label || d.incident_type,
  }));

  return (
    <Card title="Performance by Incident Type">
      {data.length === 0 ? (
        <p className="text-sm text-text-tertiary text-center py-8">No incident data yet</p>
      ) : (
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={formatted} layout="vertical" margin={{ left: 10 }}>
              ...
              <YAxis type="category" dataKey="name" stroke={colors.chart.axis} fontSize={10} width={120} axisLine={false} tickLine={false} />
              <Tooltip
